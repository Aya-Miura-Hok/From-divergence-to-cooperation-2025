...
# Subset fungal taxa and relevant sample types
...
pseq_phylum <- exp %>%
  aggregate_rare(level = "Phylum", detection = 5, prevalence = 0.18) %>%
...
# Collapse rare taxa and aggregate by Description
pseq_genus <- exp %>%
  aggregate_rare(level = "Genus", detection = 5, prevalence = 0.28) %>%
  microbiome::transform(transform = "compositional") %>%
