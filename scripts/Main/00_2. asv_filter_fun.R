...
# ============================================================
sample_sheet <- read.csv("data/fungi/metadata_fun_grouped.csv", row.names = 1)
asv_sheet <- read.csv("data/fungi/table_fun_grouped.csv", row.names = 1)
asv_sheet<-t(asv_sheet)
tax_sheet <- read.csv("data/fungi/tax_table.csv", row.names = 1)
...
if (!taxa_are_rows(otu_raw)) mat_raw <- t(mat_raw)  # 行=ASV, 列=sample
...
saveRDS(exp_kept, "outputs/exp_kept_fun.rds")
saveRDS(ps_final, "outputs/ps_final_fun.rds")
ps_alpha <- exp_kept
ps_beta  <- ps_final